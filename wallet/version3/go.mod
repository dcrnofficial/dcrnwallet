module github.com/Decred-Next/dcrnwallet/wallet/version3/v8

go 1.12

require (
	github.com/Decred-Next/cspp/v8 v8.0.0
	github.com/Decred-Next/dcrnd/blockchain/stake/version2/v8 v8.0.8
	github.com/Decred-Next/dcrnd/blockchain/standalone/v8 v8.0.8
	github.com/Decred-Next/dcrnd/blockchain/v8 v8.0.8
	github.com/Decred-Next/dcrnd/chaincfg/chainhash/v8 v8.0.8
	github.com/Decred-Next/dcrnd/chaincfg/v8 v8.0.8
	github.com/Decred-Next/dcrnd/dcrec/secp256k1/version2/v8 v8.0.8
	github.com/Decred-Next/dcrnd/dcrec/v8 v8.0.8
	github.com/Decred-Next/dcrnd/dcrutil/version2/v8 v8.0.8
	github.com/Decred-Next/dcrnd/gcs/version1/v8 v8.0.8
	github.com/Decred-Next/dcrnd/hdkeychain/version2/v8 v8.0.8
	github.com/Decred-Next/dcrnd/rpc/jsonrpc/types/version1/v8 v8.0.8
	github.com/Decred-Next/dcrnd/txscript/version2/v8 v8.0.8
	github.com/Decred-Next/dcrnd/wire/v8 v8.0.8
	github.com/Decred-Next/dcrnwallet/deployments/version2/v8 v8.0.0
	github.com/Decred-Next/dcrnwallet/errors/version2/v8 v8.0.1
	github.com/Decred-Next/dcrnwallet/rpc/client/dcrd/version1/v8 v8.0.0
	github.com/Decred-Next/dcrnwallet/rpc/jsonrpc/types/version14/v8 v8.0.8
	github.com/Decred-Next/dcrnwallet/validate/version1/v8 v8.0.1
	github.com/decred/go-socks v1.1.0
	github.com/decred/slog v1.0.0
	go.etcd.io/bbolt v1.3.3
	golang.org/x/crypto v0.0.0-20210711020723-a769d52b0f97
	golang.org/x/sync v0.0.0-20210220032951-036812b2e83c
)
