module github.com/Decred-Next/dcrnwallet

go 1.13

require (
	github.com/Decred-Next/cspp/v8 v8.0.9
	github.com/Decred-Next/dcrnd/addrmgr/v8 v8.0.12
	github.com/Decred-Next/dcrnd/blockchain/stake/version31/v8 v8.0.12
	github.com/Decred-Next/dcrnd/blockchain/standalone/v8 v8.0.12
	github.com/Decred-Next/dcrnd/blockchain/v8 v8.0.12
	github.com/Decred-Next/dcrnd/certgen/v8 v8.0.12
	github.com/Decred-Next/dcrnd/chaincfg/chainhash/v8 v8.0.12
	github.com/Decred-Next/dcrnd/chaincfg/v8 v8.0.12
	github.com/Decred-Next/dcrnd/connmgr/v8 v8.0.12
	github.com/Decred-Next/dcrnd/dcrec/secp256k1/version31/v8 v8.0.12
	github.com/Decred-Next/dcrnd/dcrec/v8 v8.0.12
	github.com/Decred-Next/dcrnd/dcrjson/version3/v8 v8.0.12
	github.com/Decred-Next/dcrnd/dcrutil/version31/v8 v8.0.12
	github.com/Decred-Next/dcrnd/dcrutil/version4/v8 v8.0.12
	github.com/Decred-Next/dcrnd/gcs/version1/v8 v8.0.12
	github.com/Decred-Next/dcrnd/hdkeychain/version31/v8 v8.0.12
	github.com/Decred-Next/dcrnd/rpc/jsonrpc/types/version1/v8 v8.0.12
	github.com/Decred-Next/dcrnd/txscript/version2/v8 v8.0.12
	github.com/Decred-Next/dcrnd/txscript/version31/v8 v8.0.12
	github.com/Decred-Next/dcrnd/txscript/version4/v8 v8.0.12
	github.com/Decred-Next/dcrnd/wire/v8 v8.0.12
	github.com/Decred-Next/go-socks/v8 v8.0.9
	github.com/Decred-Next/slog/v8 v8.0.9
	github.com/golang/protobuf v1.3.2
	github.com/gorilla/websocket v1.4.1
	github.com/jessevdk/go-flags v1.4.0
	github.com/jrick/bitset v1.0.0
	github.com/jrick/logrotate v1.0.0
	github.com/jrick/wsrpc/v2 v2.3.4
	go.etcd.io/bbolt v1.3.3
	golang.org/x/crypto v0.0.0-20210711020723-a769d52b0f97
	golang.org/x/sync v0.0.0-20210220032951-036812b2e83c
	golang.org/x/xerrors v0.0.0-20190717185122-a985d3407aa7
	google.golang.org/grpc v1.25.1
)

replace (
	github.com/Decred-Next/dcrnd/chaincfg/v8 => ../dcrnd/chaincfg
	github.com/Decred-Next/dcrnd/wire/v8 => ../dcrnd/wire

)